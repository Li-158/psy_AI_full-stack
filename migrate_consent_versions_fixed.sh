...
# 檢查 Docker 服務是否在運行
if ! docker-compose ps | grep -q "psychology_lab_db.*Up"; then
...
# 複製遷移腳本到容器中
docker cp backend/scripts/add_consent_versions.sql psychology_lab_db:/tmp/

# 在容器中執行遷移
echo "正在執行資料庫遷移..."
docker-compose exec -T postgres psql -U psychologylab -d psychologylab -f /tmp/add_consent_versions.sql
