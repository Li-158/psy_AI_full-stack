...
# 查看容器名稱
echo "檢查容器狀態..."
docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "(postgres|db)"

# 使用容器名稱執行 SQL
CONTAINER_NAME=$(docker ps --format "{{.Names}}" | grep -E "(postgres|psychology.*db)" | head -1)

if [ -z "$CONTAINER_NAME" ]; then
    echo "錯誤：找不到 PostgreSQL 容器"
    exit 1
fi
...
# 執行 SQL
docker exec -i $CONTAINER_NAME psql -U psychologylab -d psychologylab << 'EOF'
...
-- 顯示創建的表
\dt consent_versions
\dt project_participants
\dt participant_consent_versions
EOF

if [ $? -eq 0 ]; then
    echo "✅ 資料庫遷移成功！"
    echo "正在重啟後端服務..."
    docker restart $(docker ps --format "{{.Names}}" | grep -E "backend|api" | head -1)
    echo "✅ 完成！您現在可以使用同意書版本管理功能了。"
    echo ""
    echo "功能說明："
    echo "- 同意書版本現在綁定在計畫層級，而非子計畫"
    echo "- 每個計畫可以有多個同意書版本"
    echo "- 參與者可以簽署一個或多個版本"
else
    echo "❌ 遷移失敗"
